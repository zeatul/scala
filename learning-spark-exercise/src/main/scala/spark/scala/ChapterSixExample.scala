...
object ChapterSixExample {
  ...
  def main(args: Array[String]) {
    ...
    //foreach action
    file.foreach(line => if (line.contains("KK6JKQ")) { count += 1 })
    file.foreach(_.contains("KK6JKQ") match { case true => count += 1; case false => }) //两种写法
    ...
    /**
     * Lookup the countries for each call sign for the contactCounts RDD.
     * We load an array of call sign prefixes to country code to support this lookup.
     * We use broadcast to share variable in work node to improve performance
     */
