...
public class ChapterSixExample {
	...
	public static class VerifyCallLogs implements
			Function<CallLog[], CallLog[]> {

		public CallLog[] call(CallLog[] input) throws Exception {
			ArrayList<CallLog> res = new ArrayList<CallLog>();
			if (input != null) {
				for (CallLog call : input) {
					if (call != null && call.mylat != null
							&& call.mylong != null && call.contactlat != null
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length != 4) {
			throw new Exception(
					"Usage AccumulatorExample sparkMaster inputFile outDirectory");
		}
		...
		JavaSparkContext sc = new JavaSparkContext(sparkMaster,
				"ChapterSixExample", System.getenv("SPARK_HOME"),
				System.getenv("JARS"));
		...
		rdd.foreach(new VoidFunction<String>() {// foreach is action
		...
		JavaRDD<String> validCallSigns = callSigns
				.filter(new Function<String, Boolean>() {

					public Boolean call(String callSign) throws Exception {
						Pattern p = Pattern
								.compile("\\A\\d?\\p{Alpha}{1,2}\\d{1,4}\\p{Alpha}{1,3}\\Z");
		...
		// Lookup the countries for each call sign in the contactCounts RDD.
		// use broadcast to share variable,which is transfer to work noe only
		// once
		final Broadcast<String[]> signPrefixes = sc
				.broadcast(loadCallSignTable());
		JavaPairRDD<String, Integer> countryContactCounts = contactCounts
				.mapToPair(
						new PairFunction<Tuple2<String, Integer>, String, Integer>() {
							public Tuple2<String, Integer> call(
									Tuple2<String, Integer> callSignCount) {
								String sign = callSignCount._1();
								String country = lookupCountry(sign,
										signPrefixes.value());
		...
		JavaPairRDD<String, CallLog[]> contactsContactLists = validCallSigns
				.mapPartitionsToPair(new PairFlatMapFunction<Iterator<String>, String, CallLog[]>() {
					...
					@Override
					public Iterable<Tuple2<String, CallLog[]>> call(
							Iterator<String> input) throws Exception {
						...
						try {
							...
							for (Tuple2<String, ContentExchange> signExchange : requests) {
								callsignQsos.add(fetchResultFromRequest(mapper,
										signExchange));
							}
						} catch (Exception e) {
						}
		...
		System.out
				.println(StringUtils.join(contactsContactLists.collect(), ","));

		// Computer the distance of each call using an external R program
		// adds our script to a list of files for each node to download with
		// this job
		String distScript = System.getProperty("user.dir")
				+ "/src/R/finddistance.R";
		String distScriptName = "finddistance.R";
		sc.addFile(distScript);
		JavaRDD<String> pipeInputs = contactsContactLists.values()
				.map(new VerifyCallLogs())
		...
		JavaDoubleRDD reasonableDistances = distanceDoubles
		...
		System.out
				.println(StringUtils.join(reasonableDistances.collect(), ","));
		sc.stop();
		System.exit(0);

	}

	static CallLog[] readExchangeCallLog(ObjectMapper mapper,
	...
	static Tuple2<String, CallLog[]> fetchResultFromRequest(
			ObjectMapper mapper, Tuple2<String, ContentExchange> signExchange) {
	...
	static String[] loadCallSignTable() throws FileNotFoundException {
		Scanner callSignTbl = new Scanner(new File(
